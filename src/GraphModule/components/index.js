...
const GraphComponent = ({
  ...
  const latestVitalValue =
    graphs?.data?.vitals.length === 1
      ? graphs?.data?.vitals[0]
      : graphs?.data?.vitals.length > 0
      ? graphs?.data?.vitals[graphs?.data?.vitals.length - 1]
      : "--";
  const latestVitalDeviation =
    graphs?.data?.deviations.length === 1
      ? graphs?.data?.deviations[0]
      : graphs?.data?.deviations.length > 0
      ? graphs?.data?.deviations[graphs?.data?.deviations.length - 1]
      : "";
  console.log(latestVitalValue);
  return (
    <>
      <ChartControls
        index={index}
